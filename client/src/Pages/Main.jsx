...
const Main = () => {
  ...
  return (
    <div className="flex items-center justify-center overflow-hidden">
      <img src="../bg.jpeg" alt="main" className="bg-cover" />
      ...
      <div className="absolute h-[60vh] w-[60vw] bg-white/10 backdrop-blur-[4px] border border-gray-200 shadow-lg p-6 rounded-lg flex items-start justify-start space-y-4 divide-y divide-slate-700 flex-col">
        ...
        <div className="w-full flex items-center justify-around pt-4">
          ...
          <div>
            <button
              className="bg-[#b1b5b2] text-white rounded-lg px-4 py-2"
      ...
      {popup ? (
        <div className="absolute h-screen w-screen bg-white/10 backdrop-blur-[4px] flex items-center justify-center">
          ...
          <div className="h-[80vh] w-[30vw] flex items-center justify-center bg-white/50 backdrop-blur-[50px] border border-gray-200 shadow-lg p-6 rounded-lg flex-col space-y-4">
            ...
            <button className="h-[5vh] w-[87%] bg-[#b1b5b2] text-white rounded-lg px-4 py-2 flex items-center justify-center" onClick={borrow}>
              Borrow
            </button>
          </div>
        </div>
      ...
      {borrowed ? (
        <div className="absolute bottom-[80px] right-[45%] text-[#b1b5b2] font-black text-4xl z-0">
          Borrowed : {amount}
        </div>
      ) : null}
    </div>
...
const Card = ({ identifier, onSelect, isSelected, myState, setMyState }) => {
  async function approve() {
    try {
      ...
      const tx = await contract.approve("0x0000000000000000000000000000000000000000", 2);
      await tx.wait();
      ...
      {!showPopup ? (
        <>
          {add ? (
            <div className="absolute left-[-6rem]">
              {text !== "Verified ✔️" ? (
                <>
                  ...
                  />
                  <button
                    className="bg-[#b1b5b2] text-white rounded-r-lg px-4"
                    onClick={postToAPI}
                  >
                    Submit
                  </button>
                ...
                <button className="bg-[#b1b5b2] text-white rounded-lg px-4">
                  @{inputValue}
                </button>
              )}
            </div>
            ...
            <button
              className="bg-[#b1b5b2] text-white rounded-lg px-4"
              onClick={() => setAdd(true)}
            >
              Add X
            </button>
